...
export const p1 = (input: string): number => processInput(input, ({winning, numbers}) => {
  const matches = numbers.count(num => winning.includes(num));
  return matches ? 2 ** (matches - 1) : 0;
})

export const p2 = (input: string): number => processInput(input, ({winning, numbers, cardCount}, index, cards) => {
  const matches = numbers.count(num => winning.includes(num));
