...
const processCategories = (categories: string[]): Category[][] =>
	categories.map((category) =>
		category.splitRows()
			.slice(1)
			.map((line) => line.split(' ').map(Number))
			.map(([dest, start, length]): Category => ({ dest, start, end: start + length, delta: dest - start, length }))
	);
